...
| POST | `/batch/observed-flags` | JSON body: `items` — each object requires `title` and **`price`** (observed listing price). Optional `id`, `year`, `set_name`. **No AutoGluon**; uses pairwise card-type ranks only. Response: `{"results":[...]}` per item with `card_type` (product/parallel only, no player name), `spread_ratio`, `cheaper_than_worse_tier` (``null`` when not a **serial** listing per classifier — inversion only applies to numbered/serial), `confidence`, `diagnostics` (includes `is_serial_listing`). Optional top-level `warnings` (e.g. `multiple_players_in_batch`). Max length defaults to 200 (`BOWMAN_OBSERVED_BATCH_MAX`). |
