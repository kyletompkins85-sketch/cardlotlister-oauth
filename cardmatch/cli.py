...
from cardmatch.pipeline import load_rows_from_csv, score_rows_to_run_dir
from cardmatch.player_index import default_checklist_path
from cardmatch.worker_fetch import fetch_multiple_runs
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument(
        "--input",
        default="",
        help="Input CSV (omit if --from-worker)",
    )
    ap.add_argument(
        "--from-worker",
        action="store_true",
        help="Fetch term_search_items from Worker (needs WORKER_BASE_URL, INTERNAL_API_KEY).",
    )
    ap.add_argument(
        "--term-search-run-id",
        action="append",
    ...
    ap.add_argument(
        "--output-dir",
        default="",
        help="Output directory (default: cardmatch/runs/<UTC>)",
    )
    ap.add_argument(
        "--checklist",
        default="",
    ...
    out_dir = Path(args.output_dir) if args.output_dir else root / "cardmatch" / "runs" / _utc_run_id()
    ...
    if args.from_worker:
        ...
        inp = Path(args.input)
        if not inp.is_file():
            print("Provide --input CSV or use --from-worker with run IDs.", file=sys.stderr)
            return 2
    ...
    try:
        full_path, slice_path, summary_path = score_rows_to_run_dir(
            rows_in=rows_in,
            out_dir=out_dir,
            checklist=checklist,
